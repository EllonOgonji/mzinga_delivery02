defmodule MzingaDeliveryWeb.OrderView do
  ...
  def render("show.json", %{order: order} = assigns) do
    response = %{data: order_json(order)}

    # Include M-Pesa response if present (when creating order)
  ...
  # Determine overall order status from order items
  defp get_order_status(order_items) do
    ...
    cond do
      ...
      Enum.any?(statuses, &(&1 == "delivered")) -> "in_progress"
      true -> "mixed"
    end
  end
